-- Off days in the final owner week may fall in the following calendar month.
-- Their adjacent assignments belong to the following month's roster and are
-- validated by validate_carry_over_off_days_on_publish(). The owner-month
-- publisher must therefore validate only off dates inside its own month.
...
begin
  ...
  from public.employee_off_days off_day
  where off_day.roster_period_id = period_row.id
    and off_day.off_date >= period_row.month_start
